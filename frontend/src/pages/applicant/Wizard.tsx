...
function LockedWorkView({ w, isLocked, saving, toast, invalidate, setSaving, api,
  ...
  const [newRole, setNewRole] = useState({ title:'', start_date:'', end_date:'' });
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add New Role/Title + Update End Date buttons */}
      ...
      {showNewRole && (
        <div className="border-2 border-blue-200 rounded-xl p-3 bg-blue-50/20 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="To (MM/YYYY)" required>
              <DateInput value={newRole.end_date} onChange={v=>setNewRole((p:any)=>({...p,end_date:v}))} placeholder="Present"/>
            </Field>
          </div>
          <div className="flex gap-2">
            <button disabled={saving||!newRole.title||!newRole.start_date||!newRole.end_date}
              className="btn-primary text-xs disabled:opacity-50"
              onClick={async()=>{
                setSaving(true);
                try {
                  // Append new role to work history notes / title via updateWorkHistory
                  const updated = { ...w, title: newRole.title, start_date: newRole.start_date, end_date: newRole.end_date };
                  await api.updateWorkHistory(w.id, updated);
                  await invalidate();
                  setShowNewRole(false);
                  setNewRole({title:'',start_date:'',end_date:''});
                  toast('New role saved');
          ...
          {!showEditClientForm ? (
            ...
            <div className="border border-blue-200 rounded-xl p-3 bg-blue-50/20 mt-2 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Field label="Client company" required><input value={clientForm.client_name} onChange={(e:any)=>setClientForm((p:any)=>({...p,client_name:e.target.value}))} className="input"/></Field>
                <Field label="Role at client" required><input value={clientForm.role_at_client} onChange={(e:any)=>setClientForm((p:any)=>({...p,role_at_client:e.target.value}))} className="input"/></Field>
              </div>
              <div className="grid grid-cols-2 gap-2">
...
export default function ApplicantWizard() {
  ...
  const addWork = async () => {
    ...
    setSaving(true);
    try { await applicantApi.addWorkHistory(workForm); await invalidate(); setWorkForm({employment_type:'Full-time Employee',employer_name:'',title:'',employer_country:'United States',employer_state:'',employer_city:'',start_date:'',end_date:'Present',work_arrangement:'direct',client_engagements:[]}); setDateErrors({}); toast('Work history saved'); }
    catch(e:any){ toast(e.response?.data?.detail||'Error','error'); }
    finally { setSaving(false); }
  };
      ...
      <div className="card">
                ...
                <div className="mt-4">
                  ...
                  {showAuthUpdate && (
                    <div className="mt-3 border-2 border-blue-200 rounded-xl p-4 bg-blue-50/20 space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button type="button" disabled={saving || !authUpdate.legal_status || !authUpdate.prev_end_date || !authUpdate.new_start_date}
                          className="btn-primary text-xs disabled:opacity-50"
                          onClick={async()=>{
                            setSaving(true);
                            try {
                              await applicantApi.updateDemographics({
                              ...
                              toast('Authorization status updated');
            ...
            {(data?.education||[]).map((e:any) => (
              <EntryCard key={e.id}
                ...
                onToggle={()=>{
                  ...
                  else{setExpandedEntry(e.id);setEditingEdu(isLocked?null:{...e});}
                }}>
            ...
            {(data?.work_history||[]).map((w:any) => (
              <EntryCard key={w.id}
                ...
                onToggle={()=>{
                  ...
                  else{setExpandedWorkId(w.id);if(!isLocked)setEditingWork({...w,client_engagements:w.client_engagements||[]});}
                }}>
        ...
        {step===3 && (
          <div>
            ...
            {(data?.references||[]).map((r:any) => (
              <EntryCard key={r.id}
                ...
                onToggle={()=>{
                  ...
                  else{setExpandedEntry(r.id);setEditingRef(isLocked?null:{...r});}
                }}>
            ...
            <AddSection label="Reference" onSave={addRef} onCancel={()=>setRefForm({referee_first_name:'',referee_last_name:'',referee_email:''})} saving={saving}
        ...
        {step===4 && (
          <div>
            ...
            {(data?.certifications||[]).map((cert:any) => (
              <EntryCard key={cert.id}
                ...
                onToggle={()=>{
                  ...
                  else{setExpandedEntry(cert.id);setEditingCert(isLocked?null:{...cert});}
                }}>
                ...
                {editingCert?.id===cert.id && !isLocked && (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
            ...
            <AddSection label="Certification" onSave={addCert} onCancel={()=>setCertForm({cert_name:''})} saving={saving}
